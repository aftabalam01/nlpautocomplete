...
def train_one_epoch(automodel, device, optimizer, train_loader, lr, epoch, log_interval):
    ...
    for batch_idx, (data, label) in enumerate(tqdm.tqdm(train_loader,miniters=10)):
        ...
        optimizer.zero_grad()
        output = automodel(data)
        pred = output.max(-1)[1]
        loss = automodel.loss(output, torch.flatten(label))
    ...
    with torch.no_grad():
        ...
        for batch_idx,(data , label) in enumerate(eval_loader):
            ...
            try:
                ...
                # print(output)
                # print(label)
                eval_loss += automodel.loss(output, torch.flatten(label)).item()
                pred = torch.argmax(output, dim=1)#output.max(-1)[1]
                #correct_mask = pred.eq(label.view_as(pred))
                correct_mask = pred == torch.flatten(label)
