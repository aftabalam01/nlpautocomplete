...
class AutoCompleteNet_bilstm(nn.Module):
    def __init__(self, vocab_size, feature_size,num_layers=8):
        super(AutoCompleteNet_bilstm, self).__init__()
        self.vocab_size = vocab_size
        self.feature_size = feature_size
        self.num_layers = num_layers
        self.encoder = nn.Embedding(self.vocab_size, self.feature_size)
        self.lstm = nn.LSTM(self.feature_size, self.feature_size, batch_first=True,num_layers=num_layers, dropout=0,bidirectional=True)
        #self.decoder = nn.Linear(self.feature_size, self.vocab_size)
        #self.encoder.weight
        self.decoder = nn.Linear(self.feature_size*2, self.vocab_size)
        bidir_weights = torch.nn.Parameter(torch.cat((self.encoder.weight,self.encoder.weight),-1))
        # This shares the encoder and decoder weights as described in lecture.
        self.decoder.weight = bidir_weights 
        self.decoder.bias.data.zero_()
    ...
    def forward(self, x, hidden_state=None,cell_state=None):
        ...
        sequence_length = x.shape[1]
        if type(hidden_state) == type(None):
            hidden_state = Variable(torch.zeros(self.num_layers*2, batch_size, self.feature_size).to(device))
        if type(cell_state) == type(None):
            cell_state = Variable(torch.zeros(self.num_layers*2, batch_size, self.feature_size).to(device))
        x = self.encoder(x)
        #print(f'encode output shape {x.shape}')
        x, (hidden_state,cell_state) = self.lstm(x, (hidden_state,cell_state))
        #print(f'gru output shape {x.shape}')
        #print(f' decode weight shape {self.decoder.weight.shape}')
        x = self.decoder(x)
